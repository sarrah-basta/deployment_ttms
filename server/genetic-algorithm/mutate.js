...
module.exports = function (entity) {
  const result = [entity[0].slice(), entity[1].slice()];
  const constraints = this.userData.constraints;
  const timetable = table(...entity);
  const collisions = constraints.reduce(
    (acc, constraint) => acc.concat(findCollisions(timetable, constraint)),
    []
  );
  const randomCollision =
    collisions[Math.floor(Math.random() * collisions.length)].split(",")[
